...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        if stride != 1 or in_channels != out_channels * self.expansion:  # 이미지 크기 변경 or 채널수 증가시 1x1 conv1을 적용한 Shortcut
            self.residual = nn.Sequential(
                # 1x1 Conv은 dim의 크기를 변경하지 않고 채널수만 바뀌므로 padding = 0
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, out_channels, blocks, stride):
        ...
        strides = [stride] + [1] * (blocks - 1)   # ex) blocks = 2 : [1, 1] or [2, 1]
